...
- [x] run_timer match 분기문 별도 함수로 추출 (handle_command)
  - [x] input:&mut String -> &str로 타입 바꾸기
    - [x] input은 내용이 변할 수 있기 때문에 &str은 안된다(읽기 전용이기 때문)
  - [ ] 현재 구조상 사용자가 잘못 입력했을때 처리하는 로직이 없다.
  - [ ] Option 반환 아이디어자체는 좋았으나 내 의도와는 맞지 않는 것 같다. 
- [x] timer.state == working => is_working 함수로 변경 (캡슐화)
- [x]  입력 스트림 main으로 단일화
  - [ ]  된건가? 다시 한번 체크해보자
